...
builder.Services.AddSingleton(sp =>
{
    ...
    if (string.IsNullOrWhiteSpace(apiKey))
    {
        throw new InvalidOperationException(
            "Missing OpenAI API key. Set OPENAI_API_KEY env var or OpenAI:ApiKey config.");
    }
...
app.Logger.LogInformation(
    "OPENAI_API_KEY present: {Present}, length: {Len}",
    !string.IsNullOrWhiteSpace(envKey),
    string.IsNullOrWhiteSpace(envKey) ? 0 : envKey.Length);
...
// 12) Seed Helpers
// ---------------------------
static async Task SeedRolesAndAdminUserAsync(WebApplication app)
{
    using var scope = app.Services.CreateScope();
...
// Keep this OFF for now while debugging Render/Vercel cross-origin behavior
// app.UseHttpsRedirection();
